<template>
  <div class="container mx-auto px-4 py-6">
    ...
    <div v-else-if="pastDeliveries.length === 0" class="bg-white rounded-lg shadow p-8">
    ...
    <div v-else class="bg-white rounded-lg shadow overflow-hidden">
      <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="delivery in pastDeliveries" :key="delivery.id" class="hover:bg-gray-50">
              ...
              <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                {{ delivery.delivery_time ? formatDate(delivery.delivery_time) : '-' }}
              </td>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div v-if="delivery.ratings && delivery.ratings.length > 0" class="flex items-center">
                  <svg
                    v-for="star in 5"
                    :key="star"
                    :class="[
                      'h-4 w-4',
                      star <= getAverageRating(delivery.ratings) ? 'text-yellow-400' : 'text-gray-300'
                    ]"
                    fill="currentColor"
                    viewBox="0 0 20 20"
                  >
      ...
      <!-- Pagination -->
      <div v-if="totalPages > 1" class="bg-gray-50 px-4 py-3 flex items-center justify-between sm:px-6">
...
const viewDeliveryDetails = (deliveryId: number) => {
  router.push(`/delivery-agent/deliveries/${deliveryId}`);
};
...
const fetchPastDeliveries = async () => {
  ...
  try {
    ...
    // const response = await fetch(`/api/deliveries/past?page=${currentPage.value}&limit=${itemsPerPage.value}`);
    // const data = await response.json();
    // pastDeliveries.value = data.items;
    // totalItems.value = data.total;
    
    // Mock data for demonstration
    pastDeliveries.value = [
      {
        id: 1,
        tracking_code: 'TRK-2024-001',
        delivery_type: 'full' as any,
        status: DeliveryStatus.DELIVERED,
        pickup_time: new Date('2024-01-15T10:00:00'),
        delivery_time: new Date('2024-01-15T16:30:00'),
        delivery_agent_id: 1,
        announcement_id: 1,
        announcement: {
          id: 1,
        ...
        packages: [{}, {}, {}] as any,
        ratings: [{ score: 5 }, { score: 4 }] as any
      },
      {
        id: 2,
        tracking_code: 'TRK-2024-002',
        delivery_type: 'partial' as any,
        status: DeliveryStatus.DELIVERED,
        pickup_time: new Date('2024-01-20T09:00:00'),
        delivery_time: new Date('2024-01-20T14:00:00'),
        delivery_agent_id: 1,
        announcement_id: 2,
        announcement: {
          id: 2,
